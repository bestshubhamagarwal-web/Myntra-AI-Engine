...
def apply_migrations(database_url: str) -> list[str]:
    ...
    try:
        ...
        if "vector" in text or "extension" in text:
            raise RuntimeError(
                "CREATE EXTENSION vector failed. Enable pgvector on Neon "
                "(SQL Editor: CREATE EXTENSION vector) or use a Postgres host that "
                f"ships the extension. Original error: {exc}"
            ) from exc
        raise
    return applied
